...
float getUSDectection(uint trigPin, uint echoPin1, uint echoPin2, uint echoPin3)
{
  ...
  /* Hardcoded addition as sensors give a set wrong value */
    ultrasonic_Avg_Front = ( ultrasonic_Avg_Front / 50 )-0.5;
    ultrasonic_Avg_Left = ( ultrasonic_Avg_Left / 50 ) -1;
    ultrasonic_Avg_Right = ( ultrasonic_Avg_Right / 50 ) -0.6;

    // algo to filter out nagative values due to offset set for ultrasonic sensors
    if (ultrasonic_Avg_Front < 0 )
    {
        ultrasonic_Avg_Front = 0;
    }
    if (ultrasonic_Avg_Left < 0 )
    {
        ultrasonic_Avg_Left = 0;
    }
    if (ultrasonic_Avg_Right < 0 )
    {
        ultrasonic_Avg_Right = 0;
    }
...
float getHump()  
{
    ...
    accelo_Height = accelo_Avg_Diff / 1060.45;
